...
const { generateAccessAndRefreshTokens } = require("../routes/api/auth/index.js");
...
exports.verifyJWT = async (req, res, next) => {
  ...
  try {
    ...
    try { 
        const {user:{ _id}} = jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET);

        const {accessToken: newAccessToken, refreshToken:newRefreshToken} = generateAccessAndRefreshTokens(_id)
    }
